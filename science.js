...
xhr.open('GET', `https://newsapi.org/v2/top-headlines?category=science&language=en&apiKey=${apiKey}&pageSize=50`, true);
...
xhr2.open('GET', `https://newsapi.org/v2/top-headlines?category=science&language=en&apiKey=${apiKey2}&pageSize=50&page=2`, true);
...
function Country_search(category) {
    ...
    if (search_value2 == 0)
        xhr.open('GET', `https://newsapi.org/v2/top-headlines?category=${category}&language=en&apiKey=${apiKey}&pageSize=50`, true);
    else
        xhr.open('GET', `https://newsapi.org/v2/top-headlines?category=${category}&apiKey=${apiKey}&pageSize=50&country=${search_value2}`, true);
    xhr.onload = function () {
        if (this.status === 200) {
            let json = JSON.parse(this.responseText);
            let articles = json.articles;
            if (articles.length == 0)
                ...
                document.getElementById("heading").innerHTML = `Science News of ${document.getElementById(search_value2).innerHTML}`
            document.getElementById("loader_container").style.display = "none"
            document.getElementById("loader_container").style.visibility = "hidden"
            // console.log(articles[0].content);
            articles.forEach(function (element, index) {
    ...
    if (search_value2 == 0)
        xhr2.open('GET', `https://newsapi.org/v2/top-headlines?category=${category}&language=en&apiKey=${apiKey2}&pageSize=50&page=2`, true);
    else
        xhr2.open('GET', `https://newsapi.org/v2/top-headlines?category=${category}&apiKey=${apiKey2}&pageSize=50&page=2&country=${search_value2}`, true);
    xhr2.onload = function () {
        if (this.status === 200) {
            let json = JSON.parse(this.responseText);
            let articles = json.articles;
...
function latest_news(category) {
    ...
    xhr.open('GET', `https://newsapi.org/v2/everything?excludeDomains=reuters.com&apiKey=${apiKey}&pageSize=50&page=1&q=${category}&sortBy=popularity`, true);
    ...
    xhr2.open('GET', `https://newsapi.org/v2/everything?excludeDomains=reuters.com&apiKey=${apiKey2}&pageSize=50&page=2&q=${category}&sortBy=popularity`, true);
